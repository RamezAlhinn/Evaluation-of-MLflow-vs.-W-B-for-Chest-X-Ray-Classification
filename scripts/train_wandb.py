...
Trains the CNN model for Chest X-Ray Classification using Weights & Biases for experiment tracking
"""
...
def main():
    parser = argparse.ArgumentParser(description='Train CNN with W&B tracking')
    ...
    parser.add_argument('--project_name', type=str, default='Chest-XRay-Classification-WB',
    ...
    print("="*60)
    print("Note: Make sure you are logged in to W&B (wandb login)")
    ...
    if args.test:
        ...
        print("\nTest Set Results:")
        print(f"  Accuracy: {test_metrics['test/accuracy']:.2f}%")
        print(f"  Precision (Macro): {test_metrics['test/precision_macro']:.4f}")
        print(f"  Recall (Macro): {test_metrics['test/recall_macro']:.4f}")
        print(f"  F1 Score (Macro): {test_metrics['test/f1_macro']:.4f}")
